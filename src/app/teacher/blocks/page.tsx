    ...
    return (
        <div className="container mx-auto p-0">
            <div className="flex items-center justify-between mb-6">
                <h1 className="text-3xl font-headline font-bold">My Blocks</h1>
                <Button asChild>
                    <Link href="/teacher/blocks/new"><PlusCircle /> Create New Block</Link>
                </Button>
            </div>
                ...
                {blocks.map((block) => (
                    <Card key={block.id} className="flex flex-col">
                        ...
                        <CardContent className="flex-grow flex flex-col justify-end">
                             <div className="flex justify-between items-center text-sm text-muted-foreground mb-4">
                                ...
                                <span className="font-mono text-xs bg-muted px-2 py-1 rounded-md">{block.code}</span>
                            </div>
                            <div className="flex gap-2 mt-auto">
                                <Button variant="outline" size="sm" asChild className="flex-1">
                                    <Link href={`/teacher/blocks/${block.id}`}><BookOpen className="mr-2 h-4 w-4"/>View</Link>
                                </Button>
                 ...
                 <Card className="border-dashed border-2 hover:border-primary hover:text-primary transition-colors flex items-center justify-center">
                    <Button variant="ghost" asChild className="h-full w-full">
                        <Link href="/teacher/blocks/new" className="flex flex-col items-center justify-center gap-2 h-56">
                            <PlusCircle className="h-8 w-8" />
                            <span className="text-sm font-medium">Create New Block</span>
                        </Link>
                    </Button>
                </Card>
            </div>
